...
class ExpiresAtVO:
    ...
    def _validate_value_lt(self, incoming_time: datetime) -> None:
        if incoming_time < datetime.now(timezone.utc):
            raise ValueObjectExceptionError(
                f"Invalid value in {self._cls_name}. "
                f"Value={incoming_time.isoformat()} cannot be latest than now."
            )
    ...
    def check_value(self) -> None:
        ...
        self._validate_value_lt(incoming_time=self._value)
